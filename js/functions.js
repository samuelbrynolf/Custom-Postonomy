(function($){
	...
	if(nextPostsLink.length){
		...
		nextPostsLink.bind('tap', function(){
		  var $this = $(this);
	...
	if(linksParent.length){
		...
		var loadParent = loadTarget.parent('#js-cptWrapper');
		var targetHeight = loadTarget.outerHeight();
		console.log(targetHeight);
		
		loadParent.css('min-height', targetHeight);
		...
		links.each(function(index) {
			...
			// 1: Conditional på om filterlänken går till paginerad sida. Finns pagin? Existerar den idag?
			...
			// Klar: 1. Uppdatera container height innerHeight(); till en min-height. släng in transitions på denna parentklass
			// 2. Hur göra med css? spinners, transistions... etc.
	...
	function loadContent(state) {
		...
		function contentSwitcher() {
			loadTarget.load(url, function() {
				...
				var newHeight = loadTarget.outerHeight();
				loadParent.css('min-height', newHeight);
